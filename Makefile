.PHONY: help deploy preflight ssh-key ssh-config create-utility create-vms configure-utility configure-ceph \
        boot-control-planes monitor-installation post-install configure-odf configure-htpasswd configure-letsencrypt print-hosts cleanup startup shutdown lint check \
...
configure-letsencrypt: ## Configure valid SSL certs via Let's Encrypt (needs enable_letsencrypt)
	$(ANSIBLE_CMD) playbooks/10b-configure-letsencrypt.yml
